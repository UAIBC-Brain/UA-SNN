...
def main():
    ...
    out_dir = os.path.join(args.out_dir,
                           f'./pjs/T{args.T}_{args.batch_size}_{args.opt}_lr{args.lr}_c{args.in_channels}_drop{args.drop}_json_softmax_0.95_outout2_{args.fisher_c}')
    ...
    # pretrained_path = r'D:/rhn\mri_cls_spikformer/logs/jsb_spikeformer-unertrainty/T4_8_sgd_lr0.02_c1_drop0.1_jsb_100samples_0.01_amp_cupy/checkpoint_max.pth'
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        with torch.no_grad():
            ...
            for i_batch, batch_data in enumerate(test_loader):
                ...
                for t in range(args.T):
                    image_t = image[t]
                    out_fr += model(image_t)
                    out_fr_avg = out_fr / (t + 1)
                    out_fr_avg = out_fr_avg

                    p = F.softmax(out_fr_avg, dim=1)
                    confidence_score = p.max(dim=1)[0].item()
                    # print('confidence_score', confidence_score)

                    base_threshold = 0.95
                    adaptive_threshold = base_threshold - 0.2 * (t / args.T)
        ...
        if test_acc >= 0.7:
            with open(output_file_path, 'a') as f:  
                f.write(
                    f'(epoch:{epoch},avg_T:{avg_T},note_acc:{test_acc:.2f},SEN:{SEN:.2f},SPE:{SPE:.2f},F1:{F1_score:.2f}\n')
        checkpoint = {
